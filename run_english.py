# -*- coding: utf-8 -*-
"""English OMR pipeline: PDF/images -> 45 answers -> partial/full scoring if keys exist."""
...
def write_outputs(rows: list[dict], out: Path, stem: str, key: dict | None) -> tuple[Path, Path]:
    ...
    ws = wb.active
    ws.title = "영어 판독"
    header = ["페이지", "학교번호", "반", "번호", "성명", "부분점수", "부분만점", "부분틀린문항"] + [str(q) for q in range(1, 46)]
    ws.append(header)
    for c in ws[1]:
    ...
    xlsx_path = out / f"{stem}_영어_판독표.xlsx"
    wb.save(xlsx_path)
...
def main() -> int:
    ...
    ap.add_argument("--names", default=None, help="국어/기존 점수표 CSV for name join")
    ap.add_argument("--template", type=Path, default=None)
    ap.add_argument("--dpi", type=int, default=300)
    ap.add_argument("--out", type=Path, default=Path("output"))
    args = ap.parse_args()

    template = ec.load_template(args.template)
    key = load_english_key(args.keys_dir, args.irecord)
    if key:
        print(f"영어 키: {key.get('item_count', len(key.get('answers', {})))}문항, complete={key.get('complete', False)}")
    else:
    ...
    if pages:
        ec.oc.save_review_images(args.pdf, pages, args.out, args.dpi)
