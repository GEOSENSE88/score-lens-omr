# -*- coding: utf-8 -*-
"""Korean History OMR reader for the 2027 June evaluation-style answer sheet."""
...
def load_template(path: str | Path | None = None) -> dict:
    path = Path(path) if path else Path(__file__).with_name("templates") / "history_eval_2027_06.json"
    tmpl = json.loads(path.read_text(encoding="utf-8"))
    tmpl["objective"] = [dict(block) for block in tmpl["objective"]]
    return tmpl
...
def read_answers(gray, xf, template: dict | None = None) -> dict[int, int]:
...
def process_page(path: str, template: dict | None = None) -> dict:
    ...
    return dict(path=path, subject="한국사", answers=read_answers(gray, xf, template))
